...
suite('CodeAction', () => {
	...
	test('CodeActions are sorted by type, #38623', async function () {
		...
		const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'manual' }, CancellationToken.None);
	...
	test('getCodeActions should filter by scope', async function () {
		...
		{
			const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'auto', filter: { kind: new CodeActionKind('a') } }, CancellationToken.None);
		...
		{
			const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'auto', filter: { kind: new CodeActionKind('a.b') } }, CancellationToken.None);
			assert.equal(actions.length, 1);
			assert.strictEqual(actions[0].title, 'a.b');
		}

		{
			const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'auto', filter: { kind: new CodeActionKind('a.b.c') } }, CancellationToken.None);
	...
	test('getCodeActions should not return source code action by default', async function () {
		...
		{
			const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'auto' }, CancellationToken.None);
			assert.equal(actions.length, 1);
			assert.strictEqual(actions[0].title, 'b');
		}

		{
			const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), { type: 'auto', filter: { kind: CodeActionKind.Source, includeSourceActions: true } }, CancellationToken.None);
			assert.equal(actions.length, 1);
			assert.strictEqual(actions[0].title, 'a');
		}
	});
	...
	test('getCodeActions should not invoke code action providers filtered out by providedCodeActionKinds', async function () {
		...
		const { actions } = await getCodeActiona(model, new Range(1, 1, 2, 1), {
			type: 'auto',
			filter: {
